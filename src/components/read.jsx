...
class Read extends React.Component{
  constructor(props) {
    ...
    this.novelId = this.props.match.params.id; //书籍在列表的序号
    this.pos = _.findIndex( storejs.get('bookList') ,{'novel_name_id':parseInt(this.novelId) } )  ; //书籍在列表的序号
    ...
    return (
      <Spin className='loading' spinning={this.state.loading} tip="章节内容加载中">
        <Layout >
          ...
          <div ref='box' className={styles.box} style={readSetting} onClick={this.shwoSetting} onScroll={this.readScroll}>
          {this.state.loading ? '' : (()=>{
            return (
              <div>
                <h1>{this.state.chapter.title}</h1>
                <pre className={styles.pre } style={readSetting}>{this.state.chapter.cpContent}</pre>
      ...
      </div>
      {
        show ?  
            <Footer className={styles.footer}>
              ...
              <div><LeftOutlined onClick={preChapter}  style={{fontSize:25}}/></div>
              <div><RightOutlined onClick={nextChapter} style={{fontSize:25}}/></div>
              <div><CloudDownloadOutlined style={{fontSize:25}}/></div>
              <div onClick={() => this.showChapterList(true)}><MenuOutlined /></div>
            </Footer>
        : ''
      }
